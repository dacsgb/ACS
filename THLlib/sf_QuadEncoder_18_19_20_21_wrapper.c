/*
  ...
  *   "Wrapper S-functions".
  *
  *   Created: Fri Jun 19 18:26:39 2015
  */
...
typedef struct { int pinA; int pinB; int pos; int del; int ovf; int unf} Encoder;    
...
void irsPinAEn1(){
  ...
  if (drA == HIGH) {   /* low->high on A? */
    ...
    if (drB == LOW) {  /* check pin B */
		...
		if (Enc[1].pos == 32767) {   /*if pos is at max value and is supposed to increment again, overflow has occured */
			Enc[1].ovf++;
			Enc[1].pos = 0; /*reset encoder back to 0*/
		}

  	Enc[1].pos++;  /* going clockwise: increment         */
    } else {

		if (Enc[1].pos == -32767) {   /*if pos is at min value and is supposed to decrement again, underflow has occured */
			Enc[1].unf++;
			Enc[1].pos = 0; /*reset encoder back to 0*/
		}
    ...
    if (drB == HIGH) { /* check pin B */
  	...
  	Enc[1].pos++;  /* going clockwise: increment         */
		...
		if (Enc[1].pos == -32767) {   /*if pos is at min value and is supposed to decrement again, overflow has occured */
			Enc[1].unf++;
			Enc[1].pos = 0; /*reset encoder back to 0*/
		}
  	Enc[1].pos--;  /* going counterclockwise: decrement  */
 ...
 *
 */
void sf_QuadEncoder_18_19_20_21_Outputs_wrapper(int16_T *pos1,
                          int16_T *pos2,
                          int16_T *OF1,
                          int16_T *OF2,
                          int16_T *UF1,
...
/*
  ...
  *
  */
void sf_QuadEncoder_18_19_20_21_Update_wrapper(const int16_T *pos1,
                          const int16_T *pos2,
                          const int16_T *OF1,
                          const int16_T *OF2,
                          const int16_T *UF1,
...
if (xD[0]!=1) {
    ...
    # ifndef MATLAB_MEX_FILE
        ...
        /* Initialize Overflow and Underflow                    */
        Enc[enc[0]].ovf = 0;
        Enc[enc[0]].unf = 0;
        Enc[enc[1]].ovf = 0;
        Enc[enc[1]].unf = 0;
