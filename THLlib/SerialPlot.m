...
for k=1:1e5
    ...
    if ((abs(dat(end))>1e10)) % || ((abs(dat(end))<1e-10)&&(abs(dat(end))~=0))),
...
uicontrol('Style', 'pushbutton', 'String', 'Stop','Units','normalized','Position', [0 0 .1 .05],'Callback', @stopbutton);
...
uicontrol('Style', 'pushbutton', 'String', 'AutoScale','Units','normalized','Position', [.3 0 .2 .05],'Callback', @AutoScaleButton);
